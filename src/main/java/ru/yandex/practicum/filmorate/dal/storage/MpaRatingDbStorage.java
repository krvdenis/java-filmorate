...
public class MpaRatingDbStorage extends BaseDbStorage<MpaRating> {
    ...
    private static final String FIND_MPA_RATING_BY_FILM_ID_QUERY = "SELECT mr.* FROM mpa_rating AS mr " +
            "JOIN film AS f ON mr.mpa_rating_id = f.mpa_rating_id WHERE f.id = ?";
    private static final String COUNT_MPA_QUERY = "SELECT COUNT(mpa_rating_id) FROM mpa_rating";
