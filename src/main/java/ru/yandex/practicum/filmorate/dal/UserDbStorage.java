...
public class UserDbStorage extends BaseDbStorage<User> implements UserStorage {
    private static final String INSERT_CREATE_USER_QUERY = """
    ...
    private static final String FIND_BY_ID_QUERY = """
                ...
                WHERE id = ?
            """;
    private static final String UPDATE_LOGIN_BY_ID_QUERY = """
                UPDATE "user" SET login = ?
                WHERE id = ?
            """;
    private static final String UPDATE_NAME_BY_ID_QUERY = """
                UPDATE "user" SET name = ?
                WHERE id = ?
            """;
    private static final String UPDATE_EMAIL_BY_ID_QUERY = """
                UPDATE "user" SET email = ?
                WHERE id = ?
            """;
    private static final String UPDATE_BIRTHDAY_BY_ID_QUERY = """
                UPDATE "user" SET birthday = ?
                WHERE id = ?
            """;
    private static final String INSERT_CREATE_FRIEND_QUERY = """
                INSERT INTO user_friend (user_id, friend_user_id)
                VALUES (?, ?)
            """;
    ...
    public User createUser(User user) {
        try {
            ...
            if (errorMessage.contains("key 'login'") || errorMessage.contains("duplicate entry for key 'login'")) {
    ...
    public User updateUser(User newUser) {
        ...
        try {
            ...
            return findUserById(newUser.getId()).orElseThrow(() ->
                    new NotFoundException("После обновления данных пользователь с ID " + newUser.getId()
                            + " не найден.")
            );
        } catch (DuplicateKeyException e) {
            String errorMessage = e.getMessage().toLowerCase();
